...
## 🚨 КРИТИЧЕСКИЕ ПРАВИЛА: opencode.json и .opencode/plugins

> **Doc-ID:** OPENCODE-CONFIG-1 | Дата: 2026-06-04 | Связанные: `~/.config/opencode/opencode.jsonc`, `TestQA/opencode.json`, `TestQA/.opencode/plugins/`

### Что произошло 2026-06-04 (реальный кейс)

Сессия `ses_17795c709ffe2whWKALxn2QOXC` при попытке "улучшить MCP-конфиг" внесла **необратимые на тот момент** изменения:

1. Переписала `TestQA/opencode.json` в **VS Code формате** (`mcpServers` вместо `mcp`, `command: "node"` + `args: [...]` + `cwd: "..."`).
2. Создала **два параллельных плагина** `.opencode/plugins/auto-commit.js` и `.opencode/plugins/autocommit.js`, оба экспортируют `AutoCommit`.

**Последствия:** после этой сессии **ни одна TestQA-сессия не могла отправить сообщение в чат** (TUI зависал на 30+ секунд), MCP-серверы перестали загружаться, каждое действие ассистента вызывало 2-3 конфликтующих `git commit`. Восстановление заняло отдельную сессию с откатом БД.

### ⚠️ ПРАВИЛО #1: `opencode.json` — формат ТОЛЬКО opencode, НЕ VS Code

| ❌ НЕЛЬЗЯ (VS Code / Claude Desktop) | ✅ ПРАВИЛЬНО (opencode) |
|---|---|
| `"mcpServers": { ... }` | `"mcp": { ... }` |
| `"command": "node"` (строка) | `"command": ["node", "path/to/server.js"]` (массив) |
| `"args": ["arg1", "arg2"]` | Аргументы — внутри массива `command` |
| `"cwd": "..."` | **Не поддерживается** — используй абсолютные пути |
| `"env": { "KEY": "val" }` | `"environment": { "KEY": "val" }` (camelCase) |

**Эталон (правильный конфиг):**
```json
{
  ...
  "mcp": {
    "my-server": {
      "type": "local",
      "command": ["node", "Q:\\User_Data\\Desktop\\TestQA\\mcp-servers\\my-server\\server.js"],
      "enabled": true
    }
  }
}
...
**Почему критично:** opencode **молча игнорирует** `mcpServers`. Локальный `opencode.json` имеет приоритет над глобальным, поэтому если в нём `mcpServers` — глобальные MCP-серверы (`browser`, `qtest-debug`, `chrome-devtools`, `mcp-opencode-db`) **тоже не загрузятся**. Сломается вся работа с браузером, БД и отладкой.

### ⚠️ ПРАВИЛО #2: `.opencode/plugins/*.js` — ОДИН файл = ОДИН уникальный export

| ❌ НЕЛЬЗЯ | ✅ ПРАВИЛЬНО |
|---|---|
| `auto-commit.js` + `autocommit.js` (оба `export const AutoCommit`) | Только **один** файл с уникальным именем экспорта |
| Несколько плагинов делают `git add && git commit` на одно событие | Каждый плагин делает ровно одно действие |

**Проверить перед коммитом:**
```bash
ls TestQA/.opencode/plugins/   # должен быть ОДИН .js файл
```

**Почему критично:** оба плагина слушают `tool.execute.after` для `edit/write/apply_patch` и запускают `git add -A && git commit`. Получается 2-3 коммита на одно изменение + watcher `fs.watch` на `.git` (даже если `.git` в IGNORE) → race condition, блокировка stdout, зависание TUI.

### ⚠️ ПРАВИЛО #3: пути в `opencode.json` — абсолютные Windows-пути

| ❌ НЕЛЬЗЯ | ✅ ПРАВИЛЬНО |
|---|---|
| `q:/User_Data/...` (нижний регистр + `/`) | `"Q:\\User_Data\\..."` (верхний + `\\`) |
| Относительные пути (`./mcp-servers/...`) | Только абсолютные |
| `Q:/...` (смешанно) | `Q:\\...` (только backslash) |

### Если сломал — чек-лист восстановления

1. **Backup БД** (ОБЯЗАТЕЛЬНО перед любыми правками):
   ```powershell
   Copy-Item "C:\Users\misch\.local\share\opencode\opencode.db" "C:\Users\misch\.local\share\opencode\opencode-backup-$(Get-Date -Format yyyy-MM-dd).db"
   ```
2. **Проверь JSON:** `Get-Content opencode.json -Raw | ConvertFrom-Json`
3. **Проверь плагины:** в `.opencode/plugins/` должен быть **ОДИН** файл
4. **Проверь схему:** поле `mcp` (не `mcpServers`), `command` — массив, нет `cwd`
5. **Перезапусти opencode** в проекте
6. **Тест:** отправь короткое сообщение в чат — должно ответить за < 5 сек

### Запомни

> **Любая правка `opencode.json` или `.opencode/plugins/` без понимания opencode-схемы = поломка всего проекта. Если не уверен — НЕ ТРОГАЙ, спроси пользователя.**
...
> Полная документация: [Composite Steps →](/composite-steps) на VitePress-сайте.
