...
# Статус проекта qtest-runner — Июнь 2026

> **Source:** `status.md`

## Тесты (Phase 4 завершена)

| Пакет | Тестов | Статус |
|-------|--------|--------|
| browser-agent (action-parser) | 59 | ✅ |
| browser-agent (executor) | 51 | ✅ |
| browser-agent (ws-server) | 19 | ✅ |
| recorder-service (convertToSteps) | 76 | ✅ |
| **api-gateway** | **35** | ✅ |
| **step-library-service** | **27** | ✅ |
| **execution-service** | **12** | ✅ |
| **zephyr-client** | **10** | ✅ NEW |
| **Итого** | **289** | **8/8 files pass** |

## Выполненные фазы

| Фаза | Статус | Что сделано |
|------|--------|-------------|
| **Phase 0** | ✅ | Исследование Jira, PAT, multi-profile `~/.qtest/credentials.json` |
| **Phase 1** | ✅ | Config Loader (`loadCredentials` с BOM handling) |
| **Phase 2** | ✅ | Zephyr Sync (реальный импорт TC, пагинация, folder tree) |
| **Phase 3** | ✅ | API Gateway missing routes (6 новых) |
| **Phase 4** | ✅ | Unit tests (+10, всего 289) |
| **Phase 5** | ✅ | Документация VitePress, Chrome Ext, Jira Auth |
| **Phase 6** | ✅ | Minor fixes (drag dupes, selection, HTTPS fetch, BOM) |
| **MCP Tools** | ✅ | 3 сервера (browser 12, qtest-debug 10, chrome-devtools 8) |
| **Web UI** | ✅ | SyncPage (5 вкладок, дерево папок, фильтры, пагинация, Test Cycles detail, Diff, Coverage); TC Detail (3 таба, execution history, Zephyr-стиль) |
| **E2E Pipeline** | ✅ | Sync, Test Runs, Folder Tree, Filters, Open in Jira, Publish Recorder → Zephyr |

## MCP Серверы

| Сервер | Инструментов | Назначение |
|--------|-------------|-----------|
| mcp-browser | 12 | Браузер через Playwright |
| mcp-qtest-debug | 10 | Отладка, запись, тесты |
| mcp-chrome-devtools | 8 | CDP-анализ SPA (Chrome DevTools) |

## Zephyr Scale API

| Эндпоинт | Статус |
|----------|--------|
| `GET /rest/tests/latest/testcase/search` | ✅ 200 (пагинация, 7331 TC в IBPA) |
| `GET /rest/tests/latest/testrun/search` | ✅ 200 (тестовые прогоны) |
| `GET /rest/api/latest/project` | ✅ 200 (100+ проектов) |
| `POST /api/zephyr/sync` | ✅ Импорт TC в локальную БД |
| `POST /api/zephyr/sync/testrun` | ✅ Импорт TC из прогона |
...
- **Таймауты:** Все fetch-вызовы имеют `AbortSignal.timeout(15000)` (кроме `qtest_health` — 5s)
...
| Video recording (optional) | ✅ | `recordVideo` param in `launchSession()`, `POST /api/launch`, MCP tools |
...
## Планируемые фичи (из Electron Debug MCP)

| Фича | Приоритет | Где реализовать | Статус |
|------|-----------|-----------------|--------|
| **Event Stream Viewer** | высокий | `web-ui/src/pages/EventStreamPage.tsx` + `POST /api/events/capture` | ⬜ |
| **CDP в browser-agent** | высокий | `browser-agent/` — Network.enable для response body | ⬜ |
| **Persistent browser session** | средний | `browser-agent/` — один browser между вызовами | ⬜ |
| **Action Replay Editor** | средний | `web-ui/src/pages/ReplayPage.tsx` + `POST /api/replay/run` | ⬜ |
| **DOM Inspector** | низкий | `web-ui/src/components/DOMInspector.tsx` — data-tp viewer | ⬜ |

### Event Stream Viewer
Живая таблица событий браузера (console, network, DOM, errors) с фильтрацией.

**API:** `POST /api/events/capture {types, durationMs}` → использует CDP из `mcp-chrome-devtools`
**UI:**
- Вкладки: Console | Network | DOM | Errors
- Фильтры по типу, URL, тексту
- Кнопка "Start"/"Stop"
- Экспорт в JSON

### CDP в browser-agent
Добавить к Playwright page events CDP-подписку Network.enable для полного перехвата response body.

**Сейчас:** Playwright перехватывает только request/response без тела.
**После:** CDP даёт `Network.responseReceived` с полным body.

### Persistent Session
Один browser instance на всё время работы, вместо нового на каждый MCP вызов.

### Action Replay Editor
Визуальный редактор JSON-сценариев с drag & drop шагов.
...
```json
{
  ...
  "steps": [
    ...
    {"action": "assert_text", "selector": ".error", "contains": "Invalid"}
  ]
}
```
