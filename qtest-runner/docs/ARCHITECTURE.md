...
| **browser-agent** | `packages/browser-agent/` | — | 3005 | CDP, profiles, Playwright browser |
| **api-gateway** | `packages/api-gateway/` | — | 3000 | BFF, routing, aggregation |
| **web-ui** | `packages/web-ui/` | — | 8080 | React/Vite frontend |
| **chrome-extension** | `packages/chrome-extension/` | — | — | Manifest V3, action panel |
| **stub-site** | `packages/stub-site/` | — | 3006, 9091 | Static test pages |
| **mcp-browser** | `packages/mcp-browser/` | — | stdio | MCP tools for browser control |
| **mcp-qtest-debug** | `packages/mcp-qtest-debug/` | — | stdio | MCP tools for debug & record |
| **mcp-chrome-devtools** | `packages/mcp-chrome-devtools/` | — | stdio | CDP-based SPA analysis |
...
│  recordVideo: { dir: 'videos', size: 1440x900 } (опционально)│
...
### mcp-chrome-devtools (анализатор SPA)

| Инструмент | Описание |
|------------|----------|
| `cdp_navigate` | Навигация в Chrome через CDP |
| `cdp_screenshot` | Скриншот |
| `cdp_evaluate` | Выполнение JS в контексте страницы |
| `cdp_get_html` | Получение HTML страницы или элемента |
| `cdp_click` | Клик по элементу |
| `cdp_get_text` | Получение текста (полный или из селектора) |
| `cdp_list_tabs` | Список открытых вкладок |
| `cdp_get_api_calls` | Перехват XHR/fetch запросов |
