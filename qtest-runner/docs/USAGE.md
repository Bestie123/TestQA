...
9. [Веб-интерфейс (темы, настройки, Docs)](#веб-интерфейс-темы-настройки-docs)
...
| Клик по canvas (x=150, y=200) | Шаг: Нажать на canvas "..." по координатам (150, 200) |
| Выделение текста на странице | Шаг: Выделить текст "..." [длина=42] |
...
**Видеозапись сессии:** Включена по умолчанию при запуске браузера (Playwright `recordVideo`).
Можно отключить параметром `recordVideo: false` в `POST /api/launch`, WebSocket `launch`, или MCP инструментах `qtest_launch_browser`/`qtest_test_course`.
Видео сохраняется как `<sessionId>.webm` при остановке записи. Доступно через API:
- `GET /api/videos` — список видео
- `GET /api/video/download?file=<sessionId>.webm` — скачивание
...
## Веб-интерфейс (темы, настройки, Docs)

Подробное описание веб-интерфейса — на отдельной странице:

- [Web UI →](/web-ui)

Кратко:

- **7 тем оформления:** Светлая, Тёмная, OpenCode, Хакерская, Фиолетовая, Океан, Закат — переключение в навбаре
- **Настройки:** 69 тогглов типов действий + режим Drag & Drop (smart/simple)
- **Docs:** встроенный iframe с документацией VitePress на порту 5173
