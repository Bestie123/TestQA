...
taskkill /fi "windowtitle eq qtest-docs:5173" /f >nul 2>&1
REM Also kill orphaned node processes on qtest ports
for /f "tokens=5" %%p in ('netstat -aon ^| findstr ":3000 :3001 :3002 :3003 :3004 :3005 :3006 :8080 :5173" ^| findstr "LISTENING"') do taskkill /pid %%p /f >nul 2>&1
...
start "qtest-docs:5173" cmd /c "title qtest-docs:5173 && cd /d %~dp0 && npx vitepress dev docs --port 5173 --host"
echo [8] docs (VitePress)  : http://localhost:5173
timeout /t 2 /nobreak >nul

start "qtest-stub:3006" cmd /c "title qtest-stub:3006 && cd /d %~dp0packages\stub-site && node server.js"
echo [9] stub-site         : http://localhost:3006 / http://localhost:9091
...
echo Stub:       http://localhost:3006 / 9091
echo.
...
taskkill /fi "windowtitle eq qtest-web:8080" /f >nul 2>&1
taskkill /fi "windowtitle eq qtest-docs:5173" /f >nul 2>&1
taskkill /fi "windowtitle eq qtest-stub:3006" /f >nul 2>&1
echo Done.
timeout /t 2 /nobreak >nul
