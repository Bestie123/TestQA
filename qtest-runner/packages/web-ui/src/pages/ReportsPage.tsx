...
export function ReportsPage({ api }: Props) {
  ...
  useEffect(() => {
    setError(null);
    fetchReportSummary(api).then(setSummary).catch((e: any) => setError(e.message || 'Ошибка загрузки'));
    fetchReportHistory(api).then(setHistory).catch((e: any) => setError(e.message || 'Ошибка загрузки'));
  }, [api]);
  ...
  return (
    <div className="reports-page">
      ...
      {!summary && !error && <p className="empty-state">Нет данных о выполнениях</p>}
    </div>
  );
}
