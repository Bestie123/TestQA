...
export function ExecutionPage({ api, executionId, onBack }: Props) {
  ...
  if (!executionId) return (
    <div style={{ padding: 24, textAlign: 'center' as const }}>
      <h2>Выполнения тест-кейсов</h2>
      <p style={{ color: '#888', marginTop: 16 }}>
        Нет активного выполнения. Запустите тест-кейс со страницы списка или деталей.
      </p>
      <button className="btn" onClick={onBack} style={{ marginTop: 12 }}>Назад к списку</button>
    </div>
  );
