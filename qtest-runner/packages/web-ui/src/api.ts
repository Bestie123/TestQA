export interface TestCase {
  ...
  created_at: string;
  updated_at: string;
  import_date: string; // ISO timestamp with second precision
}
...
export async function fetchTestCases(
  api: string,
  params?: { folder?: string; search?: string; limit?: number; offset?: number }
  ...
  return sj(fetch(url), { data: [], total: 0 });
}
...
export async function syncFromZephyrTestRun(api: string, testRunKey: string): Promise<{ fetched: number; imported: number; errors: string[] }> {
  return postJson(api, '/zephyr/sync/testrun', { testRunKey });
}

export interface TestRun {
  id: number; key: string; projectId?: number; name: string; description?: string;
  status: string; totalTestCases: number; totalExecuted: number;
  totalPass: number; totalFail: number; createdBy?: string;
  startedOn?: string; completedOn?: string;
  ...
  issueCount?: number;
  testResultStatuses?: { pass: number; fail: number; skipped: number; blocked: number; unexecuted: number } | null;
  owner?: string;
  updatedOn?: string;
  updatedBy?: string;
}

export async function fetchZephyrTestRuns(api: string): Promise<TestRun[]> {
  return sj(fetch(`${api}/zephyr/testruns`), []);
}

export async function fetchZephyrTestRunTestCases(api: string, runKey: string): Promise<{ data: any[]; total: number }> {
  return sj(fetch(`${api}/zephyr/testruns/${encodeURIComponent(runKey)}/testcases`), { data: [], total: 0 });
}

export async function fetchZephyrTestPlans(api: string): Promise<{ id: number; key: string; name: string; status: string }[]> {
  return sj(fetch(`${api}/zephyr/testplans`), []);
}

export async function fetchZephyrProjects(api: string): Promise<{ key: string; name: string; id: string }[]> {
  return sj(fetch(`${api}/zephyr/projects`), []);
}
...
export async function fetchTestCaseExecutions(api: string, key: string): Promise<Execution[]> {
  return sj(fetch(`${api}/reports/test-case/${encodeURIComponent(key)}`), []);
}
...
// ── Health ──
export async function fetchHealth(api: string): Promise<{ status: string; service: string } | null> {
  return sj(fetch(`${api}/health`), null);
}

export async function publishToZephyr(api: string, data: {
  name: string; projectKey?: string; folder?: string; priority?: string;
  precondition?: string; objective?: string; labels?: string[];
  steps: { action: string; testData: string; expectedResult: string }[];
}): Promise<{ key: string; name: string; status: string; error?: string } | null> {
  return sj(fetch(`${api}/zephyr/publish`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  }), null);
...
export async function testZephyrConnection(api: string): Promise<{ ok: boolean; version?: string; message: string }> {
  return sj(fetch(`${api}/zephyr/test-connection`, { method: 'POST' }), { ok: false, message: 'API unavailable' });
}
...
export async function fetchCredentials(api: string): Promise<CredentialsData> {
  return sj(fetch(`${api}/credentials`), {});
}

export async function saveCredentials(api: string, data: CredentialsData): Promise<{ ok: boolean }> {
  return sj(fetch(`${api}/credentials`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
...
// ── Inject TP IDs ──
export async function getInjectTp(api: string): Promise<{ injectTp: boolean }> {
  const settings = await getSettings(api);
  return { injectTp: settings.injectTp === 'true' };
}

export async function setInjectTp(api: string, enabled: boolean): Promise<{ ok: boolean; injectTp: boolean }> {
  await setSettingsBulk(api, { injectTp: enabled ? 'true' : 'false' });
  return { ok: true, injectTp: enabled };
}
