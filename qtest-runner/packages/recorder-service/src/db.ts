...
function initSchema(): void {
  ...
  const migrations = [
    ...
    'ALTER TABLE recorded_actions ADD COLUMN length INTEGER DEFAULT 0',
...
export function convertToSteps(sessionId: string): ConvertedStep[] {
  ...
  for (const a of session.actions) {
    switch (a.actionType) {
      ...
      case 'selection':
        steps.push(makeStep(a,
          `Выделить текст "${a.value || ''}" [длина=${a.length || ''}]`,
          '',
          'Текст выделен'
        ));
        break;
