...
const httpServer = createServer(async (req, res) => {
  ...
  if (url === '/api/launch' && req.method === 'POST') {
    try {
      ...
      const recordVideo = body.recordVideo !== false;
      const profile = createProfile(profileName, userDataDir);
      await launchSession(profile, { recordVideo });
  ...
  if (url === '/api/record/start' && req.method === 'POST') {
    try {
      const body = JSON.parse(await readBody(req));
      const { profileId, sessionId, recorderUrl, injectTp } = body;
      ...
      await recStart(profileId, sessionId, recorderUrl, { injectTp });
      json(res, 200, { ok: true, sessionId, injectTp: !!injectTp });
    } catch (err: any) {
...
async function handleMessage(clientId: string, msg: any, ws: WebSocket) {
  ...
  switch (msg.type) {
    case 'launch': {
      ...
      const recordVideo = msg.recordVideo !== false;
      const profile = createProfile(profileName, userDataDir);
      await launchSession(profile, { recordVideo });
