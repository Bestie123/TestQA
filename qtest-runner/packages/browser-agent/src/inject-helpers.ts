...
// ══════════════════════════════════════════════════════════════
// TP_ID_GENERATOR — скрытая генерация data-tp и data-tp-path
// на всех элементах для инструментов автоматизации.
// Сайт не видит эти атрибуты (data-* не влияют на CSS/JS).
// Включается через window.__qtestInjectTp = true перед инжектом.
// ══════════════════════════════════════════════════════════════
export const TP_ID_GENERATOR = `
// ── TP ID Generator: data-tp + data-tp-path ──
(function() {
  if (!window.__qtestInjectTp) return;

  var __tpProcessed = typeof WeakSet !== "undefined" ? new WeakSet() : null;
  var __tpCounters = {};
  var __tpObserver = null;

  function __getTpPath(el) {
    if (!el || el === document.body) return "body";
  ...
  function __getTpId(el) {
    if (!el || !el.tagName) return null;
    var tag = el.tagName.toLowerCase();
    if (!__tpCounters[tag]) __tpCounters[tag] = 0;
    __tpCounters[tag]++;
    return tag + "-" + __tpCounters[tag];
  }

  function __shouldTag(el) {
    if (!el || !el.tagName) return false;
    if (el.nodeType !== 1) return false;
    var tag = el.tagName.toLowerCase();
    if (tag === "script" || tag === "style" || tag === "meta" || tag === "link" || tag === "noscript") return false;
    if (tag === "head" || tag === "html" || tag === "body") return false;
    if (el.hasAttribute("data-tp") || el.hasAttribute("data-tp-path")) return false;
    return true;
  }

  function __tagElement(el) {
    if (!__shouldTag(el)) return;
    if (__tpProcessed && __tpProcessed.has(el)) return;
    var tpId = __getTpId(el);
    var tpPath = __getTpPath(el);
    if (tpId) {
      el.setAttribute("data-tp", tpId);
      el.setAttribute("data-tp-path", tpPath);
      if (__tpProcessed) __tpProcessed.add(el);
    }
  }

  function __tagTree(root) {
    if (!root) return;
  ...
  function __tpInit() {
    if (document.body) {
      __tagTree(document.body);
    }
  }

  function __tpStartObserver() {
    if (__tpObserver) return;
    try {
      __tpObserver = new MutationObserver(function(mutations) {
        for (var i = 0; i < mutations.length; i++) {
          var m = mutations[i];
          if (m.type === "childList") {
          ...
          if (m.type === "attributes" && (m.attributeName === "class" || m.attributeName === "style")) {
            if (m.target && m.target.nodeType === 1 && __shouldTag(m.target)) {
              __tagElement(m.target);
            }
          }
        }
      });
      __tpObserver.observe(document.documentElement, {
        childList: true,
        subtree: true,
        attributes: true,
  ...
  window.__qtestTpInit = __tpInit;
  window.__qtestTpTagElement = __tagElement;
})();
`;
