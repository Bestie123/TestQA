...
export async function launchSession(profile: BrowserProfile, options?: { recordVideo?: boolean }): Promise<Session> {
  const opts = { recordVideo: true, ...options };
  if (opts.recordVideo) {
  ...
  const context = await chromium.launchPersistentContext(profile.userDataDir, {
    ...
    ...(opts.recordVideo ? { recordVideo: { dir: videoDir, size: { width: 1440, height: 900 } } } : {}),
  });
