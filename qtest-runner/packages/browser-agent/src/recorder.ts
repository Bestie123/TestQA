...
// INJECT_SCRIPT теперь в recorder-shared (общий для browser-agent + mcp-browser)
import { INJECT_SCRIPT } from 'recorder-shared';
// Локальные хелперы сохранены (backup + TP_ID_GENERATOR используется в LOCAL_INJECT_SCRIPT)
import {
...
// ══════════════════════════════════════════════════════════════
// LOCAL_INJECT_SCRIPT — локальная копия (backup)
// Основной INJECT_SCRIPT импортирован из recorder-shared выше
// ══════════════════════════════════════════════════════════════
...
function postJson(url: string, data: any, timeoutMs = 10000): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => { if (!settled) { settled = true; clearTimeout(timer); req.destroy(); reject(new Error('Timeout')); } });
    req.write(body);
    req.end();
  });
}
...
export async function startRecording(profileId: string, sessionId: string, recorderUrl: string, options?: { cdpCapture?: boolean; cdpOptions?: CDPListenerOptions; injectTp?: boolean }): Promise<void> {
  const session = getSession(profileId);
  if (!session) throw new Error('No active browser session');

  console.log(`[recorder] startRecording session=${sessionId.slice(0,8)} profile=${profileId.slice(0,8)} recorderUrl=${recorderUrl} cdp=${!!options?.cdpCapture} injectTp=${!!options?.injectTp}`);
  ...
  if (options?.injectTp) {
    // Set flag for CURRENT page via evaluate (addInitScript only works for future navigations)
    try {
      await page.evaluate('window.__qtestInjectTp = true;');
      console.log(`[recorder] __qtestInjectTp = true (current page)`);
    } catch {}
    // Set flag for FUTURE navigations via addInitScript
    try {
      await page.addInitScript('window.__qtestInjectTp = true;');
      console.log(`[recorder] __qtestInjectTp = true (addInitScript)`);
    } catch {}
  }
  try {
  ...
  const onPageCreated = async (newPage: Page) => {
    ...
    if (options?.injectTp) {
      // Set flag for CURRENT new tab via evaluate
      try {
        await newPage.evaluate('window.__qtestInjectTp = true;');
      } catch {}
      // Set flag for FUTURE navigations via addInitScript
      try {
        await newPage.addInitScript('window.__qtestInjectTp = true;');
      } catch {}
    }
...
export async function stopRecording(sessionId: string): Promise<string | null> {
  ...
  // Collect pending actions before deleting
  const pendingActions = [...rec.pendingActions];
  rec.pendingActions = [];
  recordings.delete(sessionId);
  console.log(`[recorder] recording stopped — session=${sessionId.slice(0,8)}`);

  // Background: flush actions + save video (non-blocking)
  const profileId = rec.profileId;
  (async () => {
    try {
      if (pendingActions.length > 0) {
        const url = `${rec.recorderUrl}/api/recordings/${sessionId}/actions`;
        console.log(`[recorder] background flush ${pendingActions.length} actions → ${url}`);
        await postJson(url, { actions: pendingActions }).catch((err: any) => {
          console.log(`[recorder] background flush FAILED: ${err.message}`);
        });
      }
    } catch {}
    try {
      const session = getSession(profileId);
      if (session) {
        const videoPath = await saveVideo(session.page, sessionId);
        if (videoPath) console.log(`[recorder] video saved: ${videoPath}`);
      }
    } catch (e: any) {
