...
const RECORDER_URL = process.env.QTEST_RECORDER_URL || "http://localhost:3004";
...
async function ensureRecording() {
  ...
  try {
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (text.startsWith("__QTEST_ACTION__") && actionQueue?.isActive()) {
        try {
          const action = JSON.parse(text.slice("__QTEST_ACTION__".length));
...
server.tool(
  ...
  async ({ url }) => {
    ...
    try {
      ...
      recordProgrammatic(buildNavigateAction(url));
...
server.tool(
  ...
  async ({ selector }) => {
    ...
    try {
      ...
      recordProgrammatic(buildClickAction(selector));
...
server.tool(
  ...
  async ({ selector, text }) => {
    ...
    try {
      ...
      recordProgrammatic(buildFillAction(selector, text));
...
server.tool(
  ...
  async ({ code }) => {
    ...
    try {
      ...
      recordProgrammatic(buildEvaluateAction(code, resultStr));
...
server.tool(
  ...
  async ({ key }) => {
    ...
    try {
      ...
      recordProgrammatic(buildKeypressAction(key));
...
server.tool(
  "record_step",
  "Mark current moment as a test case step (experimental). Creates a step marker in the recording.",
  {
    stepNumber: z.number().describe("Step number"),
    description: z.string().describe("Step description in Russian"),
    expectedResult: z.string().optional().describe("Expected result"),
  },
  async ({ stepNumber, description, expectedResult }) => {
    recordProgrammatic(buildStepMarker(stepNumber, description, expectedResult));
    return { content: [{ type: "text", text: `Step ${stepNumber}: ${description}` }] };
  }
);
...
server.tool(
  "record_action",
  "Record a manual verification/observation action (experimental).",
  {
    action: z.string().describe("Description in Russian"),
    type: z.enum(["verify", "observe", "note", "assert"]).optional().describe("Action type"),
    selector: z.string().optional().describe("CSS selector of checked element"),
    expected: z.string().optional().describe("Expected value"),
    actual: z.string().optional().describe("Actual value (auto-read from selector if omitted)"),
    screenshot: z.boolean().optional().describe("Take screenshot"),
  },
  async ({ action, type, selector, expected, actual, screenshot }) => {
    await ensureBrowser();
    let actualValue = actual;
    if (selector && !actual) {
      try {
        actualValue = await page!.evaluate((sel) => {
          const el = document.querySelector(sel);
    ...
    recordProgrammatic(buildManualAction(action, type || "verify", selector, expected, actualValue || undefined, screenshotData));
    return { content: [{ type: "text", text: `[${type || "verify"}] ${action}` }] };
  }
);
...
server.tool(
  "record_start",
  "Start recording browser actions to recorder-service",
  {
    sessionId: z.string().describe("Recording session ID from recorder-service"),
    recorderUrl: z.string().optional().describe("Recorder service URL"),
  },
  async ({ sessionId, recorderUrl }) => {
    const url = recorderUrl || RECORDER_URL;
    actionQueue = new ActionQueue(url);
    actionQueue.start(sessionId);
    await ensureBrowser();
    await ensureRecording();
    return { content: [{ type: "text", text: `Recording started: session=${sessionId}, recorder=${url}` }] };
  }
);
