...
| 4 | **Бесконечная рекурсия console → зависание** | `__record()` вызывала `console.debug()`, который был перехвачен monkey-patch'ем, который снова вызывал `__record()` | Использовать `__origConsole.debug` (оригинал) в `__record()` для отправки данных в Node.js через page.on('console'). `packages/recorder-shared/src/inject-script.ts:83` |
| 5 | **Потеря последних действий при record_stop** | `stop()` вызывала `flushWithRetries()` без `await`, `sessionId = null` выполнялся до flush'а | Сделать `stop()` async и дожидаться `flushWithRetries`. `packages/recorder-shared/src/action-queue.ts:28` |
