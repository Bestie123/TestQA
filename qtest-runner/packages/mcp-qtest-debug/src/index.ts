...
server.tool(
  ...
  "Launch a browser instance via browser-agent for recording/testing",
  {
    ...
    recordVideo: z.boolean().optional().describe("Enable video recording (default: true)")
  },
  async ({ profileName, recordVideo }) => {
    try {
...
server.tool(
  ...
  async ({ courseName, recordVideo }) => {
    ...
    try {
      ...
      const launchResult = await httpPost(`${AGENT_URL}/api/launch`, { profileName: "Auto", recordVideo });
