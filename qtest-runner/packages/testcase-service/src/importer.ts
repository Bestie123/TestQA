...
export function saveTestCaseLocally(params: {
  key: string;
  name: string;
  folder?: string;
  status?: string;
  ...
  const importDate = new Date().toISOString(); // Import date with second precision
  const { key, name, folder, status, precondition, objective, priority, projectKey, steps } = params;
  db.prepare(`INSERT INTO test_cases (id, key, name, folder, status, precondition, objective, priority, project_id, created_at, updated_at, import_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(id, key, name, folder || '', status || 'Draft', precondition || '', objective || '', priority || 'Normal', projectKey || '', now, now, importDate);
  for (let i = 0; i < steps.length; i++) {
    db.prepare(`INSERT INTO test_steps (id, test_case_id, idx, action, test_data, expected_result) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(uuid(), id, i, steps[i].action, steps[i].testData, steps[i].expectedResult);
  }
  return id;
}
...
export function importTestCases(parsed: ExcelParseResult): {
  ...
  const insertTc = db.prepare(`
    INSERT INTO test_cases (id, key, name, status, precondition, objective, folder,
      priority, component, labels, owner, estimated_time, coverage_issues, coverage_pages, project_id, import_date)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const transaction = db.transaction(() => {
    for (const tc of parsed.testCases) {
      ...
      const importDate = new Date().toISOString(); // Precision to seconds: 2026-06-04T12:30:45.000Z
      insertTc.run(
        tcId, tc.key, tc.name, tc.status, tc.precondition, tc.objective,
        tc.folder, tc.priority, tc.component, tc.labels, tc.owner,
        tc.estimatedTime, tc.coverageIssues, tc.coveragePages, tc.projectId || '', importDate
      );
